...
import type { Options } from '@hono-rate-limiter/cloudflare';

export class WorkersKVStore<E extends Env = Env, P extends string = string, I extends Input = Input> implements Store<E, P, I> {
	/**
	...
	async increment(key: string): Promise<ClientRateLimitInfo> {
		...
		if (record && new Date(record.resetTime!).getTime() > Date.now()) {
			payload = {
				totalHits: record.totalHits + 1,
				resetTime: record.resetTime ? new Date(record.resetTime) : payload.resetTime,
			};
		}

		// Clamp the expiration date to a minimum of 60 seconds
		const expiration = Math.max(Date.now() + 60_000, payload.resetTime.getTime()) / 1000;
